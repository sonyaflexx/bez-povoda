                ...
                {[1, 2, 3].map((item) => (
                    ...
                    <Link href={`https://instagram.com/bezpovoda_flowers72`} target="_blank" className="overflow-hidden group font-magnet-regular leading-none flex flex-col items-center gap-2 w-full max-w-[377px]">
                        <div className="relative w-full h-[461px]">
                            ...
                            <div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center text-white text-lg font-public-sans opacity-0 group-hover:opacity-100 transition-opacity duration-300">БОЛЬШЕ</div>
                        </div>
                    </Link>
                    </li>
                ))}
                </ul>
                <Link href={`https://instagram.com/bezpovoda_flowers72`} target="_blank" className="bg-white w-[266px] h-[66px] flex items-center justify-center font-magnet-regular text-[#717171] text-[20px] pt-1 hover:bg-[#717171] hover:text-white transition-colors duration-300">Присоединиться</Link>
            </div>
            </Container>
      </section>
    )
